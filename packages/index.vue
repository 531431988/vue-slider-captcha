<template>
  <div class="vue-slider-captcha">
    ...
    <div class="vue-slider-captcha-control" @mousemove="onDragMoving" @mouseup="onDragFinish" @mouseleave="onDragFinish" @touchmove="onDragMoving" @touchend="onDragFinish">
      <div class="vue-slider-captcha-progress-bar" ref="progressBar" :style="`background: ${color}`">{{value ? successTip : ''}}</div>
      <div class="vue-slider-captcha-tip" :style="{color: isMoving && '#fff'}">{{value ? '' : tip}}</div>

      <div ref="slider" class="vue-slider-captcha-slider" @mousedown="onDragStart" @touchstart="onDragStart" style="left:0">
        <svg viewBox="0 0 1024 1024" width="40" height="40" v-if="value">
          <path d="M297.472 466.346667a43.136 43.136 0 0 0-35.882667 20.48 31.018667 31.018667 0 0 0 5.12 41.002666c51.2 51.2 102.4 107.605333 158.890667 158.890667 15.36 15.36 35.882667 15.36 56.362667-5.12 76.8-76.8 153.6-153.6 225.493333-230.613333l51.2-51.2a34.133333 34.133333 0 0 0-5.12-51.2 39.424 39.424 0 0 0-51.2 5.12l-25.6 25.6c-71.765333 76.8-148.650667 148.565333-220.202667 225.408-10.24 10.24-10.24 10.24-15.36 0l-117.888-128a28.202667 28.202667 0 0 0-25.813333-10.368z" :fill="color" p-id="2206" />
        </svg>
        <template v-else>
